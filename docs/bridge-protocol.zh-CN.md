...
后端机器通道（`cmsg-` 对话消息、`llm-` 任务）的会话身份由业务服务维护。桥接层不因 `reset_on_idle_mins` 自动切换这些对话的 Agent 上下文，避免页面仍显示原对话，但追问时 Agent 已丢失前文。普通聊天平台仍按配置执行闲置重置。

引擎通过既有 `MachineReplyChannel` 能力判定，不分析用户话术，也不硬编码平台名称。显式新建会话、业务侧上下文压缩及进程/工作区资源回收保持既有机制；保留会话不要求进程一直运行，也不会自动还原旧版本已经重置的上下文。
