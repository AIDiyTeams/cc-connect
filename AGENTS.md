...
- Delivery follows the workspace `tomako-dev-skills/docs/DELIVERY_GOVERNANCE.md`. cc-connect is a routed controller component (since 2026-09): develop on the contributor's fixed branch, freeze a pushed SHA, and promote to `test` (the controller deploys `cc-connect@test.service` and runs the management-status health gate). The workspace owner's explicit production instruction is the authorization; a deliberate `main` push then deploys `cc-connect@prod.service`. Do not bypass the controller via GitHub Actions, SSH, or manual service replacement. The pre-2026-09 manual test-runtime exception is retired and survives only as incident-run history.
