...
- CC Connect is not mapped into `tomako-deployer` and does not use environment branches. Code is committed and pushed only to this repository's `main`. The test runtime is maintained manually by an explicitly authorized infrastructure operator over SSH, following the current inventory in `Tomako-portal/deploy/SERVERS.md`: build the exact `main` SHA, preserve a rollback binary, replace only the test binary/runtime, restart only `cc-connect@test.service`, and record runtime identity, status, logs, and live-flow evidence. Do not touch `cc-connect@prod.service` without a separate explicit production instruction, and do not use GitHub Actions or a workspace release as deployment substitutes.
